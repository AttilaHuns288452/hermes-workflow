...
6. **Composio fallback for Drive** — when standard Google OAuth credentials are missing (`google_token.json` / `google_client_secret.json`) and the user has Composio access, use the Composio Google Drive path instead of blocking on OAuth. See `references/composio-drive-fallback.md`.
