...
When Python-based crawl scripts (`python3 scripts/render_page.py`) are unavailable, use `references/manual-audit-tools.md` for a browser-tool-driven manual audit covering the same categories with Hermes `browser_console`, `web_extract`, and `web_search`.
